...
\newcommand{\figcap}[1]{{\large\textbf{#1}}}
...
\begin{center}
\includegraphics[width=\linewidth,keepaspectratio]{figures/architecture_diagram.png}\\[2mm]
\figcap{図1: SAVi-DINOSAURアーキテクチャ．\\
凍結ViT $\to$ 2層MLP(384$\to$64) $\to$ Slot Attention($K$スロット) $\to$ Decoder}
\end{center}

\vspace{1mm}
\begin{itemize}[noitemsep,topsep=1mm]
\item \textbf{バックボーン}：DINOv2 ViT-S/14，DINOv1 ViT-S/16，CLIP ViT-B/16
...
\midrule
\textbf{DINOv2} & .354 & .349 & .458 & \textcolor{darkgreen}{\textbf{.651}} & .588 & .602 \\
DINOv1         & .119 & \textbf{.403} & .316 & .176 & .107 & .292 \\
CLIP           & .082 & .155 & .122 & .077 & .153 & .166 \\
\bottomrule
\end{tabular}}
\end{center}
...
\vspace{1mm}
\begin{itemize}[noitemsep,topsep=1mm]
\item \textbf{DINOv2}：K=9で FG-ARI \textcolor{darkgreen}{\textbf{0.651}}（K=3比 $+$84\%）
\item \textbf{DINOv1}：K=5ピーク（0.403）後に急崩壊（K=11で 0.107）
\item \textbf{CLIP}：全$K$で 0.08--0.17，改善なし
...
\begin{center}
\includegraphics[width=\linewidth,keepaspectratio]{figures/paper_3backbone_k11_comparison.png}\\[2mm]
\figcap{図2: K=11スロットマスク比較（同一シーン）．\\
\textbf{上段 DINOv2}：各スロットが物体・背景に対応 ／\\
\textbf{中段 DINOv1}：1スロットが支配的（スロット崩壊） ／\\
\textbf{下段 CLIP}：全マスクがほぼ均一（空間弁別なし）}
\end{center}
...
\vspace{2mm}
\begin{itemize}[noitemsep,topsep=1mm]
\item \textbf{CLIP失敗}：空間均一（cos=0.92）$\to$ 投影でさらに悪化（0.99）$\to$ 自明解に収束
...
\begin{tabular}{lcc}
\toprule
 & \textbf{DINOv2\ \ K=5} & \textbf{DINOv2\ \ K=11} \\
\midrule
FG-ARI & 0.349 & \textcolor{darkgreen}{\textbf{0.588}} \\
活性スロット & 均一・未分離 & \textcolor{darkgreen}{全11スロット分化} \\
\bottomrule
\end{tabular}}
\end{center}
...
\vspace{2mm}
\begin{center}
\includegraphics[width=\linewidth,keepaspectratio]{figures/paper_dinov2_k11_slots.png}\\[2mm]
\figcap{図3: DINOv2 K=11の3シーン可視化（金属・ゴム混在）．\\
各行：入力画像 ／ スロットセグメンテーション ／ 上位スロットheatmap}
\end{center}

\vspace{1mm}
\begin{itemize}[noitemsep,topsep=1mm]
\item \textbf{16$\times$16パッチの限界}：マスク境界がにじむ $\to$ K=11でも解消しない（ViT構造的制約）
\item 金属/ゴムの Material ARI差：\textbf{p=0.12（有意でない）}\\
...
\toprule
 & $\tau{=}0.1$ & $\tau{=}0.3$ & $\tau{=}0.5$ & $\tau{=}0.7$ & $\tau{=}1.0$ \\
\midrule
224px & .292 & .575 & .577 & \textbf{.616} & .476 \\
\textbf{448px} & .183 & \textcolor{darkgreen}{\textbf{.696}} & .475 & .462 & .557 \\
\bottomrule
\end{tabular}}
\end{center}
...
\begin{center}
\includegraphics[width=\linewidth,keepaspectratio]{figures/uncertainty_showcase.png}\\[2mm]
\figcap{図4: per-pixelエントロピーマップ（黄=高不確実性）．\\
物体境界で競合が集中 $\Rightarrow$ 物理的境界と一致}
\end{center}
...
\begin{center}
\includegraphics[width=\linewidth,keepaspectratio]{figures/temporal_scene000.png}\\[2mm]
\figcap{図5: Scene 000（IoU=0.494，最良シーン）の時系列セグメンテーション推移．\\
前半フレームで追跡維持，後半でスロット入替り発生}
\end{center}
